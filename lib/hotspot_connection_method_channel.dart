...
class MethodChannelHotspotConnection extends HotspotConnectionPlatform {
  ...
  late final Stream<RoomEvent> _roomEvents = roomEventChannel
      ...
      .map((event) {
        ...
        if (type == 'connected') {
          ...
          final peer = peerMap != null
              ? Peer.fromMap(peerMap)
              : Peer(id: map['peerId'] ?? 'unknown', name: 'Unknown');
  ...
  Future<RoomResult> createRoom(List<String> deviceIds) async {
    final result = await methodChannel.invokeMethod('createRoom', {
      'deviceIds': deviceIds,
    });
    final map = Map<String, dynamic>.from(result ?? {});

    final connected =
        (map['connected'] as List?)?.map((e) => Peer.fromMap(e)).toList() ?? [];
    final failed =
        (map['failed'] as List?)?.map((e) => e.toString()).toList() ?? [];
